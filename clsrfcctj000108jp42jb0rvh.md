...
`Artillery`를 사용해 60초간 100명의 가상 사용자가 회원 가입한다고 했을 때, Class로 작성된 Express 애플리케이션의 경우, 모든 테스트를 마쳤을 때 최종 메모리 사용량은 **101.421875MB**였으며, 함수형으로 작성된 코드의 경우에는 **101.234375MB**로 큰 차이가 없었다. 이는 Express가 안정적으로 GC를 수행하고 있음을 나타내기도 한다.
...
* 메모리 누수가 있거나 사용량이 높은 기능의 경우에는 어떻게 변경할 지를 정리하고 패턴화 시킬 필요가 있다.
